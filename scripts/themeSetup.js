...
const setupTheme = () => {
  ...
  if (!fs.existsSync(path.join(rootFolder, "onetrust"))) {
    const includesFiles = [
      "hugo.toml",
      "data",
      "content",
    ...
    const folder = createNewfolder(rootFolder, "onetrust");

    fs.readdirSync(rootFolder, { withFileTypes: true }).forEach((file) => {
      if (includesFiles.includes(file.name)) {
        if (file.isDirectory()) {
          const destination = path.join(rootFolder, "onetrust", file.name);
